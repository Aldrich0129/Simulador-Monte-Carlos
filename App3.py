...
PARAMETER_DESCRIPTIONS = {
    "S0": "**Precio Inicial (S0)** 📈\n\nPrecio actual del activo desde donde comenzará la simulación.\n\n• Ejemplo: Si Apple cotiza a $150, S0 = 150\n• Se actualiza automáticamente con el precio más reciente",
    
    "mu": "**Drift/Tendencia (μ)** 📊\n\nRetorno esperado anualizado del activo.\n\n• Rango típico: -0.5 a 0.5 (-50% a +50% anual)\n• μ = 0.10 → expectativa de +10% anual\n• μ = -0.20 → expectativa de -20% anual",
    
    "sigma": "**Volatilidad (σ)** 📉\n\nDesviación estándar anualizada de los retornos.\n\n• 0.15-0.25: Acciones estables\n• 0.30-0.50: Acciones volátiles\n• 0.60-1.00: Criptomonedas",
    
    "lambda": "**Intensidad de Saltos (λ)** ⚡\n\nFrecuencia esperada de eventos extremos por año.\n\n• λ = 1: Un salto esperado por año\n• λ = 3: Tres saltos esperados por año\n• λ = 0.5: Un salto cada dos años",
    
    "jump_mean": "**Media de Saltos** 📊\n\nMagnitud promedio del salto.\n\n• 0.1 = saltos promedio de +10%\n• -0.05 = saltos promedio de -5%\n• 0 = saltos neutros",
    
    "jump_std": "**Desviación de Saltos** 📈\n\nVariabilidad en el tamaño de los saltos.\n\n• 0.1 = saltos varían ±10% de la media\n• Mayor valor = saltos más impredecibles",
    
    "V0": "**Volatilidad Inicial (V0)** 📊\n\nVolatilidad al cuadrado en el momento inicial.\n\n• V0 = 0.04 → volatilidad inicial de 20% (√0.04)\n• Se ajusta según condiciones actuales del mercado",
    
    "kappa": "**Velocidad de Reversión (κ)** ⏱️\n\nQué tan rápido la volatilidad vuelve a su nivel promedio.\n\n• κ = 2: Reversión rápida (meses)\n• κ = 0.5: Reversión lenta (años)\n• Mayor κ = convergencia más rápida",
    
    "theta": "**Volatilidad Largo Plazo (θ)** 🎯\n\nNivel de volatilidad al que tiende el activo.\n\n• θ = 0.04: Volatilidad objetivo 20%\n• Representa el equilibrio del mercado",
    
    "sigma_v": "**Vol de Vol (σ)** 📊\n\nCuánto varía la propia volatilidad.\n\n• σ = 0.3: La volatilidad puede cambiar ±30%\n• Mayor σ = cambios más bruscos en volatilidad",
    
    "rho": "**Correlación (ρ)** 🔗\n\nRelación entre movimientos de precio y volatilidad.\n\n• ρ = -0.5: Precio baja → volatilidad sube (típico)\n• ρ = 0: Sin correlación\n• ρ = 0.5: Precio y volatilidad suben juntos",
    
    "r": "**Tasa Libre de Riesgo (r)** 💰\n\nRendimiento de inversión sin riesgo.\n\n• Bonos del gobierno\n• Valores típicos: 0.02-0.05 (2%-5% anual)\n• Afecta el drift neutral al riesgo",
    
    "n_simulations": "**Número de Simulaciones** 🎲\n\nCantidad de trayectorias diferentes a calcular.\n\n• 1,000: Rápido, precisión básica\n• 10,000: Balance precisión/velocidad\n• 50,000: Alta precisión, más lento",
    
    "time_horizon": "**Horizonte Temporal** ⏳\n\nPeríodo de tiempo a simular (en años).\n\n• 0.1 años = ~1 mes\n• 0.25 años = 3 meses\n• 1.0 años = 1 año\n• 5.0 años = largo plazo",
    
    "n_steps": "**Pasos de Tiempo** 📅\n\nNúmero de intervalos en la simulación.\n\n• 252: Un paso por día de trading\n• 52: Un paso por semana\n• 12: Un paso por mes\n• Más pasos = trayectorias más suaves",
    
    "excel_format": """**Formato de Excel** 📊

El archivo Excel debe contener:

**Opción 1 - Datos de Precios:**
• Columna 'Date' o 'Fecha': Fechas (formato YYYY-MM-DD)
• Columna 'Close' o 'Precio': Precios de cierre
• Columnas opcionales: 'Open', 'High', 'Low', 'Volume'

**Opción 2 - Datos Financieros:**
• Primera columna: Fechas o períodos
• Columnas adicionales: Ingresos, Gastos, Utilidades, etc.

**Ejemplo de estructura:**
| Date       | Close  | Volume    |
|------------|--------|-----------|
| 2024-01-01 | 150.25 | 1,234,567 |
| 2024-01-02 | 151.30 | 1,345,678 |

**Notas:**
• Puede tener múltiples hojas
• Los nombres de columnas se detectan automáticamente
• Acepta formatos .xlsx y .xls"""
}
...
def help_icon(key: str) -> str:
    ...
    description = PARAMETER_DESCRIPTIONS.get(key, "Sin descripción disponible")
    return f" ℹ️"  # Streamlit manejará el tooltip con help parameter
...
class AdvancedVisualizations:
    ...
    def create_technical_analysis_chart(data: pd.DataFrame, symbol: str) -> go.Figure:
        """Crear gráfico de análisis técnico avanzado con 5 subplots."""
        fig = make_subplots(
            rows=5, cols=1,
            shared_xaxes=True,
            ...
            row_heights=[0.4, 0.1, 0.15, 0.15, 0.2]  # Ajustado para 5 gráficos
        )

        # 1. Gráfico de Precio: Candlestick y Medias Móviles
        fig.add_trace(
            go.Candlestick(
                x=data.index,
                open=data['Open'],
        ...
        if 'SMA_20' in data.columns:
            fig.add_trace(
                go.Scatter(x=data.index, y=data['SMA_20'], mode='lines', name='SMA 20', line=dict(color='orange', width=2)),
                row=1, col=1
            )
        if 'SMA_50' in data.columns:
            fig.add_trace(
                go.Scatter(x=data.index, y=data['SMA_50'], mode='lines', name='SMA 50', line=dict(color='blue', width=2)),
                row=1, col=1
            )
        ...
        if 'RSI' in data.columns:
            fig.add_trace(
                go.Scatter(x=data.index, y=data['RSI'], mode='lines', name='RSI', line=dict(color='purple', width=2)),
                row=3, col=1
            )
        ...
        if all(col in data.columns for col in ['MACD', 'MACD_Signal', 'MACD_Histogram']):
            fig.add_trace(
                go.Scatter(x=data.index, y=data['MACD'], mode='lines', name='MACD', line=dict(color='blue', width=2)),
                row=4, col=1
            )
            fig.add_trace(
                go.Scatter(x=data.index, y=data['MACD_Signal'], mode='lines', name='Signal', line=dict(color='red', width=2)),
                row=4, col=1
            )
            macd_hist_colors = ['green' if v >= 0 else 'red' for v in data['MACD_Histogram']]
            fig.add_trace(
                go.Bar(x=data.index, y=data['MACD_Histogram'], marker_color=macd_hist_colors, name='Histograma', showlegend=False),
                row=4, col=1
            )

        # 5. Gráfico de Bandas de Bollinger
        if all(col in data.columns for col in ['BB_Upper', 'BB_Middle', 'BB_Lower']):
            fig.add_trace(
                go.Scatter(x=data.index, y=data['BB_Upper'], mode='lines', name='BB Superior', line=dict(color='gray', width=1)),
                row=5, col=1
            )
            fig.add_trace(
                go.Scatter(x=data.index, y=data['BB_Middle'], mode='lines', name='BB Media', line=dict(color='orange', width=2)),
                row=5, col=1
            )
            fig.add_trace(
                go.Scatter(x=data.index, y=data['BB_Lower'], mode='lines', name='BB Inferior', line=dict(color='gray', width=1), fill='tonexty', fillcolor='rgba(128,128,128,0.2)'),
                row=5, col=1
            )
            # Añadimos la línea de precio para dar contexto a las bandas
            fig.add_trace(
                go.Scatter(x=data.index, y=data['Close'], mode='lines', name='Precio', line=dict(color='black', width=1), showlegend=False),
                row=5, col=1
            )
        ...
        fig.update_layout(
            ...
            height=1200,  # Aumentamos la altura para acomodar el nuevo gráfico
            showlegend=True,
            xaxis_rangeslider_visible=False,
            template="plotly_white"
        )
        
        # Actualizar títulos de los ejes Y
        fig.update_yaxes(title_text="Precio", row=1, col=1)
        fig.update_yaxes(title_text="Volumen", row=2, col=1)
        fig.update_yaxes(title_text="RSI", row=3, col=1)
        fig.update_yaxes(title_text="MACD", row=4, col=1)
        fig.update_yaxes(title_text="Precio", row=5, col=1)
...
def main():
    ...
    # Inicializar estado de sesión para parámetros
    ...
    with st.sidebar:
        ...
        if asset_type == "Acciones":
            ...
            uploaded_file = st.file_uploader(
                "Cargar Excel", 
                type=['xlsx', 'xls'],
                help=PARAMETER_DESCRIPTIONS['excel_format']
            )
            symbol = None
        ...
        # Obtener precio actual si hay datos
        current_price = 100.0  # Valor por defecto
        if symbol and asset_type != "Cargar Excel":
            try:
                data_system = DataIngestionSystem()
                temp_data = data_system.fetch_stock_data(symbol, "1d")
                if not temp_data.empty:
                    current_price = float(temp_data['Close'].iloc[-1])
            except:
                pass
        ...
        if model_type == "GBM":
            ...
            with col1:
                S0 = st.number_input(
                    f"Precio Inicial (S0) {help_icon('S0')}", 
                    value=current_price,
                    help=PARAMETER_DESCRIPTIONS['S0']
                )
                mu = st.slider(
                    f"Drift (μ) {help_icon('mu')}", 
                    -0.5, 0.5, 0.1, 0.01,
                    help=PARAMETER_DESCRIPTIONS['mu']
                )
            with col2:
                sigma = st.slider(
                    f"Volatilidad (σ) {help_icon('sigma')}", 
                    0.1, 1.0, 0.3, 0.01,
                    help=PARAMETER_DESCRIPTIONS['sigma']
                )
            
            st.session_state['model_params'] = {'S0': S0, 'mu': mu, 'sigma': sigma}
            ...
            with col2:
                lam = st.slider(
                    f"Intensidad Saltos (λ) {help_icon('lambda')}", 
                    0.0, 5.0, 1.0, 0.1,
                    help=PARAMETER_DESCRIPTIONS['lambda']
                )
                jump_mean = st.slider(
                    f"Media Saltos {help_icon('jump_mean')}", 
                    -0.5, 0.5, 0.0, 0.01,
                    help=PARAMETER_DESCRIPTIONS['jump_mean']
                )
                jump_std = st.slider(
                    f"Desv. Saltos {help_icon('jump_std')}", 
                    0.0, 0.5, 0.1, 0.01,
                    help=PARAMETER_DESCRIPTIONS['jump_std']
                )
            ...
            with col1:
                ...
                V0 = st.slider(
                    f"Vol Inicial (V0) {help_icon('V0')}", 
                    0.01, 0.5, 0.04, 0.01,
                    help=PARAMETER_DESCRIPTIONS['V0']
                )
                kappa = st.slider(
                    f"Reversión (κ) {help_icon('kappa')}", 
                    0.1, 5.0, 2.0, 0.1,
                    help=PARAMETER_DESCRIPTIONS['kappa']
                )
                theta = st.slider(
                    f"Vol Largo Plazo (θ) {help_icon('theta')}", 
                    0.01, 0.5, 0.04, 0.01,
                    help=PARAMETER_DESCRIPTIONS['theta']
                )
            with col2:
                sigma = st.slider(
                    f"Vol de Vol (σ) {help_icon('sigma_v')}", 
                    0.1, 2.0, 0.3, 0.1,
                    help=PARAMETER_DESCRIPTIONS['sigma_v']
                )
                rho = st.slider(
                    f"Correlación (ρ) {help_icon('rho')}", 
                    -0.99, 0.99, -0.5, 0.01,
                    help=PARAMETER_DESCRIPTIONS['rho']
                )
                r = st.slider(
                    f"Tasa Libre Riesgo (r) {help_icon('r')}", 
                    0.0, 0.1, 0.05, 0.005,
                    help=PARAMETER_DESCRIPTIONS['r']
                )
        ...
        n_simulations = st.slider(
            f"Número de Simulaciones {help_icon('n_simulations')}", 
            1000, 50000, 10000, 1000,
            help=PARAMETER_DESCRIPTIONS['n_simulations']
        )
        time_horizon = st.slider(
            f"Horizonte (años) {help_icon('time_horizon')}", 
            0.1, 5.0, 1.0, 0.1,
            help=PARAMETER_DESCRIPTIONS['time_horizon']
        )
        n_steps = st.slider(
            f"Pasos de Tiempo {help_icon('n_steps')}", 
            50, 500, 252, 10,
            help=PARAMETER_DESCRIPTIONS['n_steps']
        )
        
        # Botón de ejecución
        run_simulation = st.button("🚀 Ejecutar Simulación", type="primary", use_container_width=True)
    ...
    with tab1:
        ...
        if symbol and asset_type != "Cargar Excel":
            with st.spinner(f"Cargando datos de {symbol}..."):
                data = pd.DataFrame() # Inicializa un DataFrame vacío
                if asset_type == "Acciones":
            ...
            # --- VERSIÓN 3: VALIDACIÓN Y CORRECCIÓN DE DATOS ---
            st.write("--- Depuración de Datos ---")
            st.write("Datos recibidos de la fuente (primeras 5 filas):")
            ...
            error_message = ""
            if data.empty:
                error_message = "No se recibieron datos de la fuente."
            elif missing_cols:
                error_message = f"Faltan las siguientes columnas requeridas: {missing_cols}"
            else:
                # Forzar conversión a tipo numérico, los errores se convertirán en NaT/NaN
                for col in ['Open', 'High', 'Low', 'Close']:
                    data[col] = pd.to_numeric(data[col], errors='coerce')
                
                # Si después de la conversión, los precios son nulos o cero, es un error
                if data['Close'].isnull().all() or (data['Close'] == 0).all():
                    error_message = "Los datos de precio son nulos o cero después de la conversión."

            if not error_message:
                st.success(f"Datos de {symbol} cargados y validados correctamente.")
                st.write("--- Fin Depuración ---")
                
                # --- El resto del código para mostrar métricas y el gráfico ---
                ...
                with col1:
                    st.metric("Precio Actual", f"${current_price:.2f}", f"{price_change:.2f}%")
                with col2:
                ...
                st.error(f"Error Crítico: {error_message}")
                st.write("--- Fin Depuración ---")

        elif uploaded_file:
            # Procesar archivo Excel (código original sin cambios)
    ...
    with tab2:
        ...
        if st.session_state.get('model_params'):
            with st.expander("📊 Parámetros Configurados", expanded=True):
                params_df = pd.DataFrame([st.session_state['model_params']])
        ...
        if run_simulation and 'market_data' in st.session_state:
            ...
            st.warning("⚠️ Primero debes cargar datos en la pestaña 'Datos y Análisis'")
    ...
    with tab3:
        ...
        if 'simulation_results' in st.session_state:
                ...
                if st.button("Ejecutar Pruebas de Estrés"):
                    with st.spinner("Ejecutando pruebas de estrés..."):
                        stress_results = monte_carlo.run_stress_test(
                            st.session_state['model_params'], stress_scenarios, model_type,
            ...
            st.info("💡 Ejecuta una simulación en la pestaña 'Simulación Monte Carlo' para ver el análisis de riesgo")
    ...
    with tab4:
        ...
        if 'market_data' in st.session_state and 'symbol' in st.session_state:
            ...
            st.info("💡 Carga datos en la pestaña 'Datos y Análisis' para ver el análisis con IA")
...
if __name__ == "__main__":
    ...
    <div style='text-align: center; color: #888;'>
        <p>Advanced Monte Carlo Financial Simulator v2.0</p>
